...
public class OggEditItem implements MetadataEditItem{
    ...
    OggEditItem(MetadataEditPage metadataEditPage, MediaItem mediaItem){
        ...
        if(metadata != null) {
            ...
            discItem = new TwoSpinnerItem(metadataEditPage, "Disc number", metadata.containsKey("disc") && !metadata.get("disc").isBlank() ? metadata.get("disc") : "", metadata.containsKey("DISCTOTAL") && !metadata.get("DISCTOTAL").isBlank() ? metadata.get("DISCTOTAL") : "", content, true);
