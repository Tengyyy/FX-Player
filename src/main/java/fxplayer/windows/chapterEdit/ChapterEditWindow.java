...
import fxplayer.*;
import fxplayer.chapters.ChapterFrameGrabberTask;
import fxplayer.mediaItems.Chapter;
...
public class ChapterEditWindow {
        ...
        editActiveProperty.addListener((observableValue, oldValue, newValue) -> {
            ...
            if (newValue) {
                ...
                if (showTransition != null && showTransition.getStatus() == Animation.Status.RUNNING)
                    showTransition.stop();
                if (hideTransition != null && hideTransition.getStatus() == Animation.Status.RUNNING)
                    hideTransition.stop();
    ...
    public void focusForward() {
        ...
        if (focus.get() >= focusNodes.size() - 1 || focus.get() == -1)
            newFocus = 0;
        else
            newFocus = focus.get() + 1;
