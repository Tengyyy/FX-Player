...
import fxplayer.*;
import fxplayer.skins.ExpandableTextArea;
...
import fxplayer.windows.mediaInformation.items.OtherItem;

import java.util.ArrayList;

public class CustomTextAreaItem extends VBox implements Component{

    public TextField keyField = new TextField();
    ...
    public CustomTextAreaItem(OtherItem otherItem, String key, String value){
        ...
        keyField.textProperty().addListener((ov, prevText, currText) -> {
            ...
            otherItem.mediaInformationWindow.changesMade.set(true);
            otherItem.mediaInformationWindow.saveAllowed.set(!currText.isEmpty());
            Platform.runLater(() -> {
        ...
        textArea.textProperty().addListener((observableValue, oldValue, newValue) -> {
            otherItem.mediaInformationWindow.changesMade.set(true);
            otherItem.mediaInformationWindow.saveAllowed.set(!newValue.isEmpty());
        });
        ...
        Platform.runLater(() -> {
            editButtonTooltip = new ControlTooltip(otherItem.mediaInformationWindow.menuController.mainController, "Edit key", "", editButton, 1000);
            removeButtonTooltip = new ControlTooltip(otherItem.mediaInformationWindow.menuController.mainController, "Remove key", "", removeButton, 1000);
            warningLabelTooltip = new ControlTooltip(otherItem.mediaInformationWindow.menuController.mainController, "Key can not be empty", "", warningLabel, 0, TooltipType.MENU_TOOLTIP);
            warningLabelTooltip.getStyleClass().add("warningLabelTooltip");
            boolean changesMade = otherItem.mediaInformationWindow.changesMade.get();
            keyField.setText(key);
            if(!changesMade) otherItem.mediaInformationWindow.changesMade.set(false);
        });
    ...
    public void removeItem(){
        ...
        otherItem.mediaInformationWindow.changesMade.set(true);
        otherItem.mediaInformationWindow.saveAllowed.set(true);
    }
