package fxplayer.windows.mediaInformation;
...
public class SavePopUp extends StackPane{
    ...
    Label label1 = new Label("Unable to save metadata");
    Label label2 = new Label("Make sure that all of the metadata keys are not-empty and unique. (no two keys can be identical)");
    ...
    public SavePopUp(MediaInformationWindow mediaInformationWindow){
        this.mediaInformationWindow = mediaInformationWindow;

        mediaInformationWindow.popupContainer.getChildren().add(this);
    ...
    public void show(){
        ...
        mediaInformationWindow.popupContainer.setMouseTransparent(false);
        this.requestFocus();
        AnimationsClass.fadeAnimation(100, mediaInformationWindow.popupContainer, 0 , 1, false, 1, true);
    }

    public void hide(){
        this.showing = false;

        mediaInformationWindow.popupContainer.setMouseTransparent(true);

        mediaInformationWindow.window.requestFocus();

        FadeTransition fadeTransition = new FadeTransition(Duration.millis(100),mediaInformationWindow.popupContainer);
        fadeTransition.setFromValue(mediaInformationWindow.popupContainer.getOpacity());
